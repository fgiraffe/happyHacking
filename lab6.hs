...
-- size_l :: [Rose a] -> int
-- size_l [] = 0
-- size_l [x : xs] = size_wha(x) + size_l(xs)
--
-- size_wha :: Rose a -> Int
-- size_wha (_ :> []) = 1
-- size_wha (_ :> [xs]) = 1 + size_l(xs)

-- size_rose_list          :: [Rose a] -> Int
-- size_rose_list []       = 0
-- size_rose_list [Rose a] = size x + size_rose_list(xs)
--
-- size :: Rose a -> Int
-- size (_ :> []) = 1
-- size (_ :> [xs]) = 1 + size_rose_list(xs)
...
main = 
    ...
    in
        do
            putStrLn (show (size rt0))
            putStrLn (show (size rt2))

--size (x :> []) = 1

-- foldr_rose          :: (a -> b -> b) -> b -> Rose a -> b
-- foldr_rose f v []   = v
-- foldr_rose f v (x :> xs) = f x (foldr_rose f v xs)
--size = foldr_rose (\ (x :> xs) -> 1 + xs) 1
